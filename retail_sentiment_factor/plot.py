...
Plots produced:
  1.  return_heatmap_vw.png     — 5x5 mean excess return heatmap (VW)
  2.  return_heatmap_ew.png     — 5x5 mean excess return heatmap (EW)
  ...
  8.  alpha_heatmap.png         — 5x5 FF6 alpha heatmap
...
def generateAllPlots(
    ...
    Args:
        portfolioReturns   : VW 5x25 portfolio returns from constructFF25Portfolios.
        portfolioReturnsEW : EW 5x25 portfolio returns from constructFF25PortfoliosEW.
        spreads            : Full-sample spreads with NW t-stats from computeSpreads.
        spreadsSmallMid    : Small/mid-cap spreads from computeSpreads.
        rollingDeltaCross  : Rolling ΔCross from computeRollingDeltaCross.
    ...
    for regime, (color, label, ls) in curve_palette.items():
        ...
        if fit.get("converged"):
            ax2.plot(fit["pe_grid"], fit["fpe_curve"],
                     ...
                     label=f"{label} (r2={fit['r_squared']:.2f})")
            any_curve = True
        else:
