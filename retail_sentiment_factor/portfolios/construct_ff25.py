...
At the end of formation month t, stocks are independently sorted into
quintiles by PE and SAT (both computed from month-t data only). The 25
intersection portfolios are held over month t+1: portfolio returns are
value-weighted (primary) or equal-weighted (robustness) averages of the
constituents' month-t+1 returns, weighted by month-t market cap.

This lag is essential: SAT is signed by month-t return direction, so a
contemporaneous sort/return pairing would be mechanically profitable
(look-ahead). The output year_month column refers to the HOLDING month
(t+1) so portfolio returns align with factor returns of the same month.
...
def constructFF25Portfolios(monthlyPanel: pd.DataFrame) -> pd.DataFrame:
    ...
    Returns:
        ...
        year_month is the holding month (formation month + 1).
    """
    _validatePortfolioInputs(monthlyPanel)
    panel = addForwardMonthlyReturns(monthlyPanel)
    panel = panel.dropna(subset=[FWD_RETURN_COL, FWD_RF_COL])
    results = (
        panel
    ...
    results["excess_ret"] = results["ret_vw"] - results[FWD_RF_COL]
    results = results.drop(columns=[FWD_RF_COL])
    return _relabelToHoldingMonth(results)
...
def constructFF25PortfoliosEW(monthlyPanel: pd.DataFrame) -> pd.DataFrame:
    ...
    results["excess_ret"] = results["ret_ew"] - results[FWD_RF_COL]
    results = results.drop(columns=[FWD_RF_COL])
    return _relabelToHoldingMonth(results)
...
def _relabelToHoldingMonth(results: pd.DataFrame) -> pd.DataFrame:
    """
    Shift the year_month label from formation month t to holding month t+1,
    so portfolio returns line up with factor returns of the same month.
    """
    results = results.copy()
    results["year_month"] = (
        pd.PeriodIndex(results["year_month"], freq="M") + 1
    )
    return results
...
def _computeOnePortfolioReturn(group: pd.DataFrame) -> pd.Series:
    """
    Value-weighted holding-month return for one (formation month, PE
    quintile, SAT quintile) cell. Weights are formation-month market cap;
    returns are month t+1 (ret_fwd).
    """
    total_me = group[MARKET_CAP_COL].sum()
    if total_me <= 0 or group.empty:
        return pd.Series({
            "ret_vw": np.nan,
            FWD_RF_COL: group[FWD_RF_COL].iloc[0] if not group.empty else np.nan,
            "n_stocks": 0,
        })
    weights = group[MARKET_CAP_COL] / total_me
    return pd.Series({
        "ret_vw":   float((weights * group[FWD_RETURN_COL]).sum()),
        FWD_RF_COL: float(group[FWD_RF_COL].iloc[0]),
        "n_stocks": len(group),
    })


def _computeOnePortfolioReturnEW(group: pd.DataFrame) -> pd.Series:
    """Equal-weighted holding-month return for one formation cell."""
    if group.empty:
        return pd.Series({
            "ret_ew": np.nan,
            FWD_RF_COL: np.nan,
    ...
    return pd.Series({
        "ret_ew":   float(group[FWD_RETURN_COL].mean()),
        FWD_RF_COL: float(group[FWD_RF_COL].iloc[0]),
        "n_stocks": len(group),
    })
