...
_MAX_FWD_GAP_DAYS: int = 7  # next obs must be within a calendar week
...
def _addForwardReturns(panel: pd.DataFrame) -> pd.DataFrame:
    ...
    Contiguity: shift(-1) alone would pair day d with the stock's next
    OBSERVED return, which after a suspension or data gap can be weeks
    later. Rows whose next observation is more than _MAX_FWD_GAP_DAYS
    calendar days ahead (one week covers weekends and holiday bridges)
    get NaN forward returns instead.
    ...
    next_date = panel.groupby("ric")["date"].shift(-1)
    gap_too_wide = (next_date - panel["date"]) > pd.Timedelta(days=_MAX_FWD_GAP_DAYS)
    panel.loc[gap_too_wide, "excess_ret_fwd"] = np.nan
