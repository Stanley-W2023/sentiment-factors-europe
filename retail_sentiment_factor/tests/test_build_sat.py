...
def test_aggregateDailyToMonthly_normalises_cross_sectionally():
    # Arrange — two stocks with distinct daily SAT levels in the same month.
    # Monthly SAT is the daily mean, then winsorised and z-scored across
    # stocks within the month, so the output cross-section has mean 0 and
    # preserves the ordering of the raw means.
    n_days = MIN_VALID_DAYS_PER_MONTH + 5
    dates = pd.bdate_range("2021-01-04", periods=n_days)
    daily = pd.concat([
        pd.DataFrame({"date": dates, "ric": "BNP.PA", "sat_daily": 0.1}),
        pd.DataFrame({"date": dates, "ric": "AIR.PA", "sat_daily": -0.1}),
    ])
    daily["year_month"] = daily["date"].dt.to_period("M")
    ...
    assert len(valid) == 2
    assert abs(valid[SAT_COL].mean()) < 1e-10
    bnp = valid[valid["ric"] == "BNP.PA"][SAT_COL].iloc[0]
    air = valid[valid["ric"] == "AIR.PA"][SAT_COL].iloc[0]
    assert bnp > air
