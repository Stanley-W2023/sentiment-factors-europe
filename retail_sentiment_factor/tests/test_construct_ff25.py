    ...
    for month in months:
        for stock_id in range(n_stocks):
            records.append({
                ...
                PE_QUINTILE_COL: (stock_id % N_QUINTILES) + 1,
...
def test_constructFF25Portfolios_returns_25_portfolios_per_month():
    ...
    n_portfolios_per_month = (
        result.groupby("year_month")[[PE_QUINTILE_COL, SAT_QUINTILE_COL]]
        .nunique()
        .prod(axis=1)
    )
...
def test_constructFF25Portfolios_value_weights_sum_to_one():
    # Arrange — sort at January (formation), hold over February.
    # Weights come from January market cap; returns from February.
    jan = pd.Period("2021-01", freq="M")
    feb = pd.Period("2021-02", freq="M")
    panel = pd.DataFrame({
        "year_month": [jan, jan, feb, feb],
        "ric": ["A", "B", "A", "B"],
        RETURN_COL: [0.50, -0.50, 0.01, 0.02],  # Jan returns must NOT matter
        MARKET_CAP_COL: [100.0, 300.0, 999.0, 999.0],  # Jan weights: 0.25, 0.75
        RISK_FREE_COL: [0.0, 0.0, 0.0, 0.0],
        PE_QUINTILE_COL: [1, 1, 1, 1],
        SAT_QUINTILE_COL: [1, 1, 1, 1],
    })
    ...
    # labelled with the holding month (February)
    expected_ret = 0.0175
    assert len(result) == 1
    assert result["year_month"].iloc[0] == feb
...
def test_computeSpreads_returns_expected_stats():
    ...
    assert any("delta_pe" in s for s in stat_names)
...
def test_computeSpreads_delta_cross_is_scalar():
    ...
    # Act
    spreads = computeSpreads(portfolios)
    delta_cross = spreads[spreads["stat"] == "delta_cross"]["mean"].values
...
def test_buildRSFLongShort_returns_one_row_per_month():
    ...
    portfolios = constructFF25Portfolios(panel)
    # The last formation month has no t+1 return, so one fewer holding month
    n_holding_months = panel["year_month"].nunique() - 1

    # Act
    rsf = buildRSFLongShort(portfolios)

    # Assert
    assert len(rsf) == n_holding_months
...
def test_constructFF25Portfolios_handles_zero_market_cap_gracefully():
    ...
    panel.loc[panel[PE_QUINTILE_COL] == 1, MARKET_CAP_COL] = 0.0

    # Act
    result = constructFF25Portfolios(panel)

    # Assert — PE quintile 1 portfolios return NaN, not raise
    pe1 = result[result[PE_QUINTILE_COL] == 1]
