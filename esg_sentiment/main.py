...
Steps:
  ...
  3. BUILD_FEATURES   — Compute SAT, ESG, and quintile assignments
...
from features.build_esg import buildESGScore
...
def main():
    print("ESF Research Pipeline — ESG Sentiment Factor")
    print(f"Sample: {SAMPLE_START} to {SAMPLE_END}")
    print(f"Regime breaks: {REGIME_BREAK_DATE_2018} (EU Action Plan), "
          f"{REGIME_BREAK_DATE_2020} (retail surge)")
    print("-" * 60)
...
def _stepBuildDailyAnalysis(
    ...
    if GENERATE_PLOTS:
        ...
        alphas      = db.readTable("ff6_alphas")
        fmb_results = db.readTable("fmb_results")
        rsf_ls      = db.readTable("esf_longshort")
...
def _stepBuildFeatures(db: Database) -> pd.DataFrame:
    print("\n[Step 3] Building SAT, ESG, and quintile features...")
    ...
    print("  Cleaning ESG scores...")
    esg_panel = _buildMonthlyESGPanel(daily_panel, db=db)
    esg_clean = buildESGScore(esg_panel)

    print("  Merging and assigning quintiles...")

    esg_clean = esg_clean.drop(columns=["me_eur"], errors="ignore")
    merged = monthly_sat.merge(esg_clean, on=["year_month", "ric"], how="inner")
...
def _stepBuildPortfolios(
    ...
    rsf_ls = buildESGSATLongShort(portfolio_returns)
    db.writeTable("esf_longshort", rsf_ls)
    print(f"\n  ESG-SAT long-short mean monthly return: "
...
def _stepRunEstimation(
    ...
    print("\n  Running Chow test (dual breaks: 2018 EU Action Plan + 2020 retail surge)...")
    # regimeBreakDate arg is ignored in the ESF version — both breaks are
    # read directly from constants inside computeChowTest. Pass 2020 break
    # for call-site compatibility with the legacy signature.
    regime_break = pd.Period(str(REGIME_BREAK_DATE_2020)[:7], freq="M")
...
def _reshapeToLongFormat(rawStoxx: pd.DataFrame) -> pd.DataFrame:
    ...
    to long (date, ric, volume, shares_outstanding, ret_eur, esg_raw, me_eur, gics_sector).
    ...
    for ric in rics:
        try:
            ...
            ric_df = ric_df.rename(columns={
                ...
                "ESG Score":                       "esg_raw",   # TR.TRESGScore
                "TRESGScore":                      "esg_raw",
            })
            parts.append(ric_df)
        except KeyError:
    ...
    for col in ("ret_eur", "volume", "shares_outstanding", "me_eur", "gics_sector", "esg_raw"):
    ...
    # Forward-fill operational fields to cover sporadic Refinitiv reporting gaps.
    # esg_raw uses a longer fill window (252 days ≈ 1 year) because ESG scores
    # are updated at most quarterly and often only annually — gaps between
    # genuine score updates are NOT missing data, they are carry-forward values.
    ...
    if "esg_raw" in long_df.columns:
        long_df["esg_raw"] = (
            long_df.groupby("ric")["esg_raw"]
            .transform(lambda s: s.ffill(limit=252))
        )
...
def _buildMonthlyESGPanel(dailyPanel: pd.DataFrame, db: "Database | None" = None) -> pd.DataFrame:
    """
    Extract monthly ESG score and GICS sector code from the pre-reshaped daily panel.

    ESG scores (TR.TRESGScore) are updated at most quarterly in Refinitiv —
    often only annually. Using 'last' aggregation within each month is correct:
    it picks up the most recent non-null score. Forward-filling within the daily
    panel (already applied in _reshapeToLongFormat for me_eur) is NOT applied to
    esg_raw here because ESG score gaps represent genuine non-disclosure rather
    than data outages — a missing month means the firm had no score, not that
    Refinitiv failed to report it. The monthly aggregation will produce NaN for
    those months, which propagates correctly to the quintile assignment step.

    If gics_sector is not present in the daily panel, falls back to the static
    GICS snapshot table exactly as _buildMonthlyPEPanel does.
    """
    long_df = dailyPanel.copy()
    long_df["year_month"] = pd.to_datetime(long_df["date"]).dt.to_period("M")

    agg_cols = {"me_eur": "last"}
    if "esg_raw" in long_df.columns:
        agg_cols["esg_raw"] = "last"
    ...
    if "esg_raw" not in monthly.columns:
        raise ValueError(
            "esg_raw not found in daily panel. "
            "Ensure TR.TRESGScore is included in STOXX_FIELDS and the data "
            "pull has been run with PULL_DATA=True."
        )

    # GICS fallback — identical to _buildMonthlyPEPanel
    if "gics_sector" not in monthly.columns and db is not None:
        if db.hasTable(TABLE_GICS):
            try:
                ...
                print(f"  [ESG panel] Joined GICS from snapshot: {n_mapped:,} stock-months mapped, "
                      f"{n_financials:,} financials (sector 40) will be excluded")
            except Exception as e:
                print(f"  [ESG panel] GICS join failed: {e}")

    if "gics_sector" not in monthly.columns:
        monthly["gics_sector"] = -1
        print(
            "  [ESG panel] WARNING: gics_sector not found — "
            "financials will NOT be excluded. Run Step 0 to pull GICS snapshot."
        )

    n_with_esg = monthly["esg_raw"].notna().sum()
    n_total = len(monthly)
    print(f"  [ESG panel] Coverage: {n_with_esg:,} / {n_total:,} stock-months "
          f"have ESG scores ({100*n_with_esg/n_total:.1f}%)")
