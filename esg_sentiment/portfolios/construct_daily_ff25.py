...
For each SAT variant (raw, 5d, 10d) we produce:
  ...
  - Spread statistics: delta_SAT(k), delta_ESG(s), delta_Cross
    ...
    Args:
        dailyPanel: Output of buildDailyAnalysisPanel. Must contain
                    [date, ric, ret_eur, me_eur, excess_ret_fwd,
                     esg_quintile, sat_raw_quintile, sat_5d_quintile,
                     sat_10d_quintile].

    Returns:
        Dict keyed by variant name ("raw", "5d", "10d"). Each value is a
        long-format DataFrame:
          [date, esg_quintile, sat_quintile, ret_vw, excess_ret_fwd_vw,
           n_stocks].
    """
    results = {}
...
def computeDailySpreads(
    ...
    Returns:
        ...
        stat values: delta_sat_k1..k5, delta_esg_s1..s5, delta_cross
    """
    records = []
...
def buildDailyRSFLongShort(
    ...
    for variant, port_df in portfolios.items():
        long_leg  = (
            port_df[port_df[ESG_QUINTILE_COL] == N_QUINTILES]
            .groupby("date")["excess_ret_fwd_vw"].mean()
        )
        short_leg = (
            port_df[port_df[ESG_QUINTILE_COL] == 1]
            .groupby("date")["excess_ret_fwd_vw"].mean()
        )
...
def _buildPortfoliosOneVariant(
    ...
    """
    Build 25 VW portfolios using ESG_QUINTILE_COL and satQCol as sort keys.
    Formation: day d characteristics → return on day d+1 (excess_ret_fwd).
    """
    needed = ["date", "ric", ESG_QUINTILE_COL, satQCol,
              MARKET_CAP_COL, "excess_ret_fwd"]
    sub = dailyPanel[needed].dropna(
        subset=[ESG_QUINTILE_COL, satQCol, "excess_ret_fwd"]
    ).copy()

    sub = sub.rename(columns={satQCol: "sat_quintile"})
    sub["esg_quintile"]  = sub[ESG_QUINTILE_COL].astype(int)
    sub["sat_quintile"] = sub["sat_quintile"].astype(int)

    results = (
        sub.groupby(["date", "esg_quintile", "sat_quintile"], group_keys=False)
...
def _buildDailySpreadTimeSeries(portDf: pd.DataFrame) -> pd.DataFrame:
    ...
    for day, grp in portDf.groupby("date"):
        ret = (
            grp.groupby(["esg_quintile", "sat_quintile"])["excess_ret_fwd_vw"]
            .mean()
        ...
        for s in range(1, N_QUINTILES + 1):
            if s in ret.columns and 5 in ret.index and 1 in ret.index:
                row[f"delta_esg_s{s}"] = ret.loc[5, s] - ret.loc[1, s]
...
def printDailySpreadSummary(spreads: pd.DataFrame):
    ...
    key_stats = (
        ...
        + [f"delta_esg_s{s}" for s in range(1, 6)]
        + ["delta_cross"]
    )
