...
At each month-end, stocks are independently sorted into quintiles by ESG score
...
def constructFF25Portfolios(monthlyPanel: pd.DataFrame) -> pd.DataFrame:
    ...
    Returns:
        ...
        [year_month, esg_quintile, sat_quintile, ret_vw, excess_ret, n_stocks].
    """
    _validatePortfolioInputs(monthlyPanel)
    results = (
        monthlyPanel
        .groupby(["year_month", ESG_QUINTILE_COL, SAT_QUINTILE_COL],
                 group_keys=False)
...
def buildESGSATLongShort(portfolioReturns: pd.DataFrame) -> pd.DataFrame:
    """
...
def _buildMonthlySpreadTimeSeries(
    ...
    Index: year_month. Columns: delta_esg_s1..s5, delta_sat_k1..k5, delta_cross.
    """
    records = {}

    for month, grp in portfolioReturns.groupby("year_month"):
        ret = (
            grp.groupby([ESG_QUINTILE_COL, SAT_QUINTILE_COL])["excess_ret"]
            .mean()
...
def _validatePortfolioInputs(panel: pd.DataFrame):
    required = {
        ...
        RISK_FREE_COL, ESG_QUINTILE_COL, SAT_QUINTILE_COL,
    }
...
def _validateSpreadInputs(portfolioReturns: pd.DataFrame):
    required = {ESG_QUINTILE_COL, SAT_QUINTILE_COL, "excess_ret"}
