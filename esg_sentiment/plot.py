"""
plots.py — All visualisations for the ESF research pipeline.
...
Plots produced:
  ...
  3.  sat_spread_ladder.png     — delta_SAT by ESG quintile with NW error bars
  4.  esg_spread_ladder.png      — delta_ESG by SAT quintile with NW error bars
  5.  cumulative_ls.png         — Cumulative ESG-SAT long-short return
...
def generateAllPlots(
    ...
    Args:
        ...
        rsfLongShort       : RSF L/S monthly returns from buildESGSATLongShort.
...
def _plotReturnHeatmap(
    ...
    mean_ret = (
        portfolioReturns
        .groupby([ESG_QUINTILE_COL, SAT_QUINTILE_COL])[ret_col]
        .mean()
    ...
    ax.set_yticklabels([f"ESG Q{i}" for i in range(1, N_QUINTILES + 1)])
    ...
    ax.set_ylabel("ESG Quintile (Q1 = Lowest ESG -> Q5 = Highest ESG)")
...
def _plotSpreadLadder(
    ...
    _drawLadder(
        ...
        prefix="delta_sat_k",
        xlabels=[f"ESG Q{i}" for i in range(1, N_QUINTILES + 1)],
        title="SAT Spread (Q5−Q1) by ESG Quintile",
        ylabel="Mean Monthly Excess Return",
    )

    # ESG spreads: delta_ESG_s1..s5
    _drawLadder(
        ax=axes[1],
        spreads_full=spreads,
        spreads_sub=spreadsSmallMid,
        ...
        title="ESG Spread (Q5−Q1) by SAT Quintile",
        ylabel="",
    )
...
def _plotCumulativeLS(rsf: pd.DataFrame, outDir: str):
    ...
    regime_str = str(pd.Period(str(REGIME_BREAK_DATE_2020)[:7], freq="M"))
    ...
    ax.set_title("ESG-SAT Long-Short Cumulative Return\n(Long ESG=5/SAT=5, Short ESG=1/SAT=1)",
                 fontsize=11)
...
def _plotRollingSharpe(rsf: pd.DataFrame, outDir: str, window: int = 36):
    ...
    _setDateTicks(ax, dates)
    ax.set_title(f"ESG-SAT Long-Short Rolling {window}-Month Sharpe Ratio (Annualised)",
                 fontsize=11)
...
def _plotRollingDeltaCross(rolling: pd.DataFrame, outDir: str):
    ...
    _setDateTicks(ax, dates)
    ax.set_title("36-Month Rolling ΔCross\n(SAT Amplification by ESG Score Over Time)",
                 fontsize=11)
...
def _plotAlphaHeatmap(alphas: pd.DataFrame, outDir: str):
    alpha_matrix = alphas.pivot(
        index=ESG_QUINTILE_COL, columns=SAT_QUINTILE_COL, values="alpha"
    ) * 100

    tstat_matrix = alphas.pivot(
        index=ESG_QUINTILE_COL, columns=SAT_QUINTILE_COL, values="t_stat"
    )
...
def _plotSmallMidCapComparison(
    ...
    for ax, prefix, title, xlabels in [
        (axes[0], "delta_sat_k",
         "SAT Spread by ESG Quintile", [f"ESG Q{i}" for i in range(1, 6)]),
        (axes[1], "delta_esg_s",
         "ESG Spread by SAT Quintile", [f"SAT Q{i}" for i in range(1, 6)]),
    ]:
        keys = [f"{prefix}{i}" for i in range(1, N_QUINTILES + 1)]
        x = np.arange(N_QUINTILES)
        width = 0.35
...
def _plotParametricSATAmplification(results: dict, outDir: str):
    """
    Two-panel plot:
      Left  — lambda_SAT by ESG quintile for full / pre / post-2020
      Right — fitted f(PE) Weibull curves for pre vs post-2020
    """
    fig, axes = plt.subplots(1, 2, figsize=(13, 5))

    # ── Left: lambda_SAT bar chart by ESG quintile, three regimes ─────────────
    ax = axes[0]
    x = np.arange(5)
    width = 0.25
    palette = {
    ...
    for regime, (color, label) in palette.items():
        if regime not in results:
            continue
        lam = results[regime]["lambda_sat_by_esg"]
    ...
    ax.set_xticks(x)
    ax.set_xticklabels([f"ESG Q{i}" for i in range(1, 6)])
    ax.set_ylabel("λ_SAT (within-PE-quintile FMB coefficient)")
    ax.set_title("SAT Premium Conditional on ESG Quintile\n"
    ...
    for regime, (color, label, ls) in curve_palette.items():
        if regime not in results:
            continue
        fit = results[regime]["fesg_fit"]
        lam = results[regime]["lambda_sat_by_esg"]

        # Scatter the observed lambda_SAT points
        valid = lam.dropna(subset=["esg_median", "lambda_sat"])
        ax2.scatter(valid["esg_median"], valid["lambda_sat"],
                    color=color, zorder=5, s=40, alpha=0.8)

        if fit.get("converged"):
            ax2.plot(fit["esg_grid"], fit["fesg_curve"],
                     color=color, linestyle=ls, linewidth=1.8,
                     label=f"{label} (r²={fit['r_squared']:.2f})")
            any_curve = True
        else:
            # Fallback: connect dots
            ax2.plot(valid["esg_median"], valid["lambda_sat"],
                     color=color, linestyle=ls, linewidth=1.5,
                     label=f"{label} (no fit)")

    ax2.axhline(0, color="black", linewidth=0.7, linestyle="--")
    ax2.set_xlabel("Median ESG Score (within quintile)")
    ax2.set_ylabel("λ_SAT")
    ax2.set_title("Parametric f(ESG): SAT Amplification Shape\n"
...
def _plotDailySpreadComparison(dailySpreads: pd.DataFrame, outDir: str):
    ...
    Left:  delta_SAT (Q5-Q1) by ESG quintile for each variant.
    Right: delta_ESG  (Q5-Q1) by SAT quintile for each variant.
    """
    fig, axes = plt.subplots(1, 2, figsize=_FIGSIZE_WIDE)
    ...
    for ax, prefix, xlabel_fn, title in [
        (axes[0], "delta_sat_k",
         lambda i: f"ESG Q{i}", "Daily SAT Spread (Q5-Q1) by ESG Quintile"),
        (axes[1], "delta_esg_s",
         lambda i: f"SAT Q{i}", "Daily ESG Spread (Q5-Q1) by SAT Quintile"),
    ]:
...
def _plotDailyFMBComparison(dailyFMB: pd.DataFrame, outDir: str):
    ...
    variables = ["esg_quintile", "sat_quintile", "esg_sat_interaction"]
