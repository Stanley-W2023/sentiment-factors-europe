...
INTERACTION_COL = "esg_sat_interaction"
...
def estimateFamaMacBeth(
    ...
    panel = stockPanel.copy()
    panel = panel.sort_values(["ric", "year_month"])
    ...
    panel[INTERACTION_COL] = (
        pd.to_numeric(panel[ESG_QUINTILE_COL], errors="coerce")
        * pd.to_numeric(panel[SAT_QUINTILE_COL], errors="coerce")
    )

    regressors = [ESG_QUINTILE_COL, SAT_QUINTILE_COL, INTERACTION_COL]
...
def computeChowTest(
    stockPanel: pd.DataFrame,
    regimeBreakDate: pd.Period,          # legacy arg — ignored, kept for call-site compat
    portfolioReturns: pd.DataFrame | None = None,
) -> pd.DataFrame:
    """
    Dual-break Chow test on the time series of monthly lambda_Int estimates.

    Tests whether the ESG×SAT interaction strengthened across two distinct
    structural breaks:

      lambda_Int_t = alpha
                     + beta_2018 * Post2018_t
                     + beta_2020 * Post2020_t
                     + epsilon_t

    Break dates (from constants):
      Post2018: Jan 2018 onwards — EU Sustainable Finance Action Plan.
                Captures institutional ESG mainstreaming.
      Post2020: Jan 2020 onwards — retail participation surge + SFDR.
                Captures incremental retail sentiment amplification.

    Interpretation:
      beta_2018 > 0 significant  → institutional ESG flows amplify interaction
      beta_2020 > 0 significant  → retail surge provides additional amplification
      Only beta_2020 significant → sentiment-driven premium, not just institutional
    """
    from config.constants import REGIME_BREAK_DATE_2018, REGIME_BREAK_DATE_2020
    ...
    for month, month_data in panel.groupby("year_month"):
        coefs = _runOneCrossSection(
            month_data,
            [ESG_QUINTILE_COL, SAT_QUINTILE_COL, INTERACTION_COL],
        )
        if coefs is not None:
    ...
    ts["post_2018"] = (ts.index > break_2018).astype(float)
    ts["post_2020"] = (ts.index > break_2020).astype(float)

    valid = ts["lambda_int"].notna()
    y = ts.loc[valid, "lambda_int"].values
    X = sm.add_constant(ts.loc[valid, ["post_2018", "post_2020"]].values)
    ...
    return pd.DataFrame({
        "variable":    ["const", "post_2018", "post_2020"],
