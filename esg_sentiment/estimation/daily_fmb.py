...
_INTERACTION_COL = "esg_sat_interaction"
...
def _estimateOneVariant(
    ...
    needed = ["date", "ric", "excess_ret_fwd", ESG_QUINTILE_COL, satQCol]
    sub = panel[needed].dropna().copy()
    sub = sub.rename(columns={satQCol: "sat_q"})

    sub["sat_q"]  = pd.to_numeric(sub["sat_q"],  errors="coerce")
    sub["esg_q"]   = pd.to_numeric(sub[ESG_QUINTILE_COL], errors="coerce")
    sub[_INTERACTION_COL] = sub["esg_q"] * sub["sat_q"]

    regressors = ["esg_q", "sat_q", _INTERACTION_COL]
...
def _printFMBSummary(results: pd.DataFrame):
    ...
    print(f"  {'Variable':<22} {'raw λ':>10} {'raw t':>8} "
          f"{'5d λ':>10} {'5d t':>8} {'10d λ':>10} {'10d t':>8}")
    print("  " + "-" * 82)

    variables = ["esg_quintile", "sat_quintile", "esg_sat_interaction", "const"]
