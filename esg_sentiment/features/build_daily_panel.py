...
def buildDailyAnalysisPanel(
    ...
    Args:
        dailyData : Long-format daily panel from _reshapeToLongFormat in main.py.
                    Required columns: [date, ric, ret_eur, me_eur, esg_raw,
    ...
    Returns:
        Daily panel with columns:
          [date, ric, ret_eur, me_eur, esg_score,
           sat_raw, sat_5d, sat_10d,
           esg_quintile, sat_raw_quintile, sat_5d_quintile, sat_10d_quintile,
...
def _mergePEAndMarketCap(
    ...
    """Merge esg_raw, me_eur, and ret_eur onto the SAT panel.
    ...
    """
    cols = ["date", "ric", "esg_raw", "me_eur", RETURN_COL]
    ...
    pe_me = pe_me.rename(columns={"esg_raw": ESG_COL})
...
def _assignDailyQuintiles(panel: pd.DataFrame) -> pd.DataFrame:
    ...
    panel[ESG_COL] = (
        panel.groupby("date")[ESG_COL]
        .transform(lambda s: s.clip(
            lower=s.quantile(0.01), upper=s.quantile(0.99)
        ))
    )
    # Exclude negative/zero PE (undefined trailing PE)
    panel.loc[panel[ESG_COL] <= 0, ESG_COL] = np.nan

    panel[ESG_QUINTILE_COL]  = panel.groupby("date")[ESG_COL].transform(_qrank)
