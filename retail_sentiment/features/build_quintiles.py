"""
build_quintiles.py — Assign PE and SAT quintile ranks for the FF-25 sort.
...
def assignQuintiles(monthlyPanel: pd.DataFrame) -> pd.DataFrame:
    ...
    Args:
        monthlyPanel: Monthly panel with columns [year_month, ric, pe_trailing,
                      sat_monthly]. Rows with NaN in either sort variable are
                      excluded from that month's sort but do not count against
                      the stock's history.

    Returns:
        Panel with additional columns [pe_quintile, sat_quintile]. Ineligible
    ...
    both_nan = panel[PE_COL].isna() & panel[SAT_COL].isna()
    dropped_nan = both_nan.sum()
    panel = panel[~both_nan]
    if dropped_nan > 0:
        print(f"  [quintiles] Dropped {dropped_nan} rows with NaN in both PE and SAT")

    panel = panel.sort_values(["ric", "year_month"]).reset_index(drop=True)

    panel["_has_data"] = (~panel[PE_COL].isna() | ~panel[SAT_COL].isna()).astype(int)
    ...
    panel[PE_QUINTILE_COL] = _assignOneQuintile(panel, PE_COL)
    panel[SAT_QUINTILE_COL] = _assignOneQuintile(panel, SAT_COL)

    return panel.dropna(subset=[PE_QUINTILE_COL, SAT_QUINTILE_COL])
...
def assignPEQuintiles(monthlyPanel: pd.DataFrame) -> pd.DataFrame:
    ...
    _validateColumn(monthlyPanel, PE_COL)
    panel = monthlyPanel.copy()
    panel[PE_QUINTILE_COL] = _assignOneQuintile(panel, PE_COL)
    return panel
...
def _validateQuintileInputs(panel: pd.DataFrame):
    required = {"year_month", "ric", PE_COL, SAT_COL}
