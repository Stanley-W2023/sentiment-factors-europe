...
def checkRawStoxx(db: Database):
    ...
    for expected in ["Total Return", "Volume", "Outstanding Shares",
                     "P/E (Daily Time Series Ratio)", "Company Market Cap"]:
    ...
    # PE coverage
    pe_coverage = sum(
        raw[ric]["P/E (Daily Time Series Ratio)"].notna().any()
        for ric in rics
        if "P/E (Daily Time Series Ratio)" in raw[ric].columns
    )
    pe_pct = pe_coverage / len(rics) * 100
    msg = f"PE data present for {pe_coverage}/{len(rics)} RICs ({pe_pct:.1f}%)"
...
def checkMonthlyPE(panel: pd.DataFrame):
    ...
    pe = pd.to_numeric(panel[PE_COL], errors="coerce")
    total   = len(pe)
    nan_pe  = pe.isna().sum()
    ...
    # Negative PE (should be zero after buildTrailingPE)
    neg_pe = (pe < 0).sum()
    if neg_pe > 0:
        _fail(f"{neg_pe:,} negative PE values — buildTrailingPE exclusion not working")
    else:
        _ok("No negative PE values")

    # Extreme PE
    extreme_pe = (pe > 200).sum()
    if extreme_pe > 0:
        _warn(f"{extreme_pe:,} PE > 200 — possible data quality issue — review raw PE values")

    # Coverage by stock
    pe_by_stock = panel.groupby("ric")[PE_COL].apply(
        lambda x: pd.to_numeric(x, errors="coerce").notna().mean() * 100
    )
    zero_coverage = (pe_by_stock == 0).sum()
    low_coverage  = (pe_by_stock < 50).sum()
    if zero_coverage > 0:
        _fail(f"{zero_coverage} stocks with ZERO PE observations — TR.PE not pulling for these")
    if low_coverage > 0:
        _warn(f"{low_coverage} stocks with < 50% PE coverage")
    _ok(f"Median PE coverage per stock: {pe_by_stock.median():.1f}%")
...
def checkQuintiles(panel: pd.DataFrame):
    ...
    for col, name in [(PE_QUINTILE_COL, "PE"), (SAT_QUINTILE_COL, "SAT")]:
    ...
    pe  = pd.to_numeric(panel[PE_QUINTILE_COL],  errors="coerce")
...
def checkPortfolios(db: Database):
    ...
    if "n_stocks" in port.columns:
        ...
        pivot = port.pivot_table(
            values="n_stocks", index=PE_QUINTILE_COL,
    ...
    matrix = port.pivot_table(
        values="excess_ret", index=PE_QUINTILE_COL,
        columns=SAT_QUINTILE_COL, aggfunc="mean"
    )
